...
{ 
 ...
 // cout<<"(E<0 || E>Q)"<<endl;
  ...
  Double_t ff=1//forbidden(Zpa,A,E,Ch,Q,n)
             *fermi(Zpa,A,E,Ch)*screening(Zpa,A,E,Ch,1.45)
            // *finitesizeEM1(Zpa,A,E,Ch)
            // *finitesizeWI(Zpa,A,E,Ch,Q)
            // *weakmag(Zpa,A,E,Ch)
...
void bi214(){
  ...
  TF1 *f3 = new TF1("f3", unique2, 0,3.276, 6);
  TF1 *f4 = new TF1("f4", allowed, 0,3.276, 6);  
  f3->SetParameter(0,Z);
  f3->SetParameter(2,A);
  ...
  Double_t binc1[3280];//记录瞬时bincontent
  Double_t binc2[3280];//记录叠加后的bincontent
  //----------初始化-------------
  for(Int_t i=0;i<=3279;i++){
    binc2[i]=0;                                                    
  }
 ...
 //-----------------forbidden-------------------------------      
  ...
  //------读入禁戒跃迁数据-----------
  Double_t data1[151][4];//输入数据，顺序：Br1,n,Q,Eg,Br2
  ifstream fin("bi214for2.dat");  
  for(Int_t m=0;m<=150;m++){
  ...
  for(Int_t m=0;m<=150;m++){    
   ...
   cout<<"Br="<<Br<<endl;
    ...
    TH1F *h1 = new TH1F("h1","h1",3276,0,3.276);
    h1->Add(f3); 
    //-----归一化   
    double entries = h1->Integral();//1796,2150
    h1->Scale(1./entries);
      
    for(Int_t i=1;i<=3276;i++){   
  ...
  //------读入允许跃迁数据-----------
  Double_t data[32][3];//输入数据，顺序：Br1,logft,Q,Eg,Br2
  ifstream fin("bi214all2.dat");  
  for(Int_t m=0;m<=31;m++){
  ...
  TH1F *h2=new TH1F("h2","Bi214 Beta Sprectrum",3276,0,3.276);
  for(Int_t i=1;i<=3276;i++){
...
const Double_t alpha1=1/137.03599976;
 ...
 Double_t y1=0.5*pow(2.*p,2.*gam-2.*gam0);
...
const Double_t alpha1=1/137.03599976;
  ...
  Double_t W=EE/E0+1> 1.00001 ? EE/E0+1 : 1.00001; //energy with elecron rest mass unit
 // Double_t W0=Q/E0+1; 
