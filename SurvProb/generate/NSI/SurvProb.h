...
class SurvProb
{
	public:
		...
		void SetupProb(double Energy, double eeel, double eeer, double etel, double eter, double eeu, double eed, double euu, double eud, double etu, double etd, double uuu, double uud, double utu, double utd, double ttu, double ttd);
		...
		double GetProbFromCalculation(int daynight, int flavor, int comp, int exp); //exp 0:jp, 1:sno, 2:sk.

	private:
		const static int model = 1; //GS98 model.
		/* Calculate and save rotate matrix to an array with dimension nsteps. */
		void SetupRotateMatrix(double Energy, int &nsteps, double* radius, MatrixXcd* Rotate);
		MatrixXcd CalculateRotateMatrix(double Energy, double radius, double L /* Normalized by sun radius */);
		MatrixXcd CalculateEarthRotate(double Energy, double ne, double nn, double np, double L /* Unit:km */);
		...
		PREM earth;
		SolarNu solar;
		double prob[2][4][9][3]; // First dimension 0 for day and 1 for night. Second dimension for e, mu, tau, sterile. Third dimension for 9 components. Fourth dimension for jinping and SNO.
		TH1D* Dist[9];
