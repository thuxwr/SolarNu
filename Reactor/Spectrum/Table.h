/*
  ...
  It will complain with extra variables than header line specifies.
  It will add 0's if insufficient contents is supplied.
  ...
  int Open( std::string filename )
  {
	...
	while( iss >> word )  {
	  if( counter>=FieldNames.size() ) {
	    std::cout<<"Extra field than the number of varialbes in header line"<<std::endl;
