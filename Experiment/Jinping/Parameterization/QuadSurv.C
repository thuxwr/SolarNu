...
int main()
{
	...
	minuit.mnparm(1, "c1", 0, 0.078, 0, 0, ierflg);
	minuit.mnparm(2, "c2", 0, 0.034, 0, 0, ierflg);
...
void probfcn(int &npar, double* gin, double &f, double* par, int iflag)
{
	TF1* func = new TF1("SurvProb", "[0]+[1]*(x-10)+[2]*(x-10)*(x-10)", 0, 20);
	for(int i=0; i<3; i++) func->SetParameter(i, par[i]);
	SurvProb = func;
	...
	//Min->FixParameter(0); //Fix C11. C11 threshold: about 2.305MeV
	//Min->FixParameter(2); //Tl208, with threshold 5.505MeV.
	//Min->FixParameter(4); //ExtTl208, with threshold about 3MeV.
	Min->FixParameter(7);
