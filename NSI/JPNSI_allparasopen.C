...
void jpfcn(int &npar, double* gin, double &f, double* par, int iflag);
...
int main(int argc, char** argv)
{
	...
	TMinuit min;
	int ierflg;
	min.mnparm(0, "sin_2_t12", 0.307, 0.1, 0.01, 0.9, ierflg);
	min.mnparm(1, "ms12", 7.5e-5, 1e-6, 6e-5, 1e-4, ierflg);
...
void jpfcn(int &npar, double* gin, double &f, double* par, int iflag)
{
	double sin_2_t12 = par[0];
	double ms12 = par[1];
	TGraph* prob = nsi.GetProb(sin_2_t12, ms12, 1, 1, ed, en);
	...
	double tan_2_t12 = sin_2_t12 / (1-sin_2_t12);
	chi2 += kam.chi2(tan_2_t12, ms12);

	f = chi2;
	delete minuit; delete prob;
	cout << "sin_2_t12  " << sin_2_t12 << "  ms12  " << ms12 << endl;
	cout << "f  " << f << endl;
}
