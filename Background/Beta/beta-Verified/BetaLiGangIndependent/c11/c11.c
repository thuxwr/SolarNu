...
Double_t allowed(Double_t *x, Double_t *par)//1.9079   1.4645e-2   1.7401
{ 
...
void c11(){
  ...
  TF1 *f3 = new TF1("f3", unique, 0,0.961, 6);
  TF1 *f4 = new TF1("f4", allowed, 0,0.961, 6);  
  f3->SetParameter(0,Z);
  f3->SetParameter(2,A);
  ...
  Double_t binc1[1990];//记录瞬时bincontent
  Double_t binc2[1990];//记录叠加后的bincontent
  //----------初始化-------------
  for(Int_t i=0;i<=1989;i++){
    binc2[i]=0;                                                    
  }
 ...
 //-----------------forbidden-------------------------------      
  ...
  for(Int_t m=0;m<=0;m++){    
    ...
    TH1F *h1 = new TH1F("h1","h1",3648,0,3.648);
    h1->Add(f3); 
    //-----归一化   
    double entries = h1->Integral();//1796,2150
    h1->Scale(1./entries);
      
    for(Int_t i=1;i<=3648;i++){   
  ...
  //------读入允许跃迁数据-----------
  Double_t data[1][3];//输入数据，顺序：Br1,logft,Q,Eg,Br2
  ifstream fin("c11all.dat");  
  for(Int_t m=0;m<=0;m++){
 ...
 //   Double_t Br2=data[m][4];
    ...
    TH1F *h1 = new TH1F("h1","h1",961,0,0.961);
    h1->Add(f4);
  ...
  TH1F *h2=new TH1F("h2","C11 Beta Sprectrum",961,0,0.961);
  for(Int_t i=1;i<=961;i++){
