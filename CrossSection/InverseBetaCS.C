//Revise Integral in line 86,111;add line 110.
...
InverseBetaCS::InverseBetaCS() {
  ...
  fR = 1.71465;
  tauN = 881.5;
  Double_t hbar = 6.58211928E-22;  // *MeV s
  Double_t c = 299792458.0;    // m/s
  fSigma0 = 2*pow(3.1415926,2)/pow(0.510998928,5)/fR/tauN/(1/hbar)/(pow(1/(hbar*c),2)) * 10000 / 1e-42;  // eqn 10, cm^2
  fSigma0 = fSigma0/fF2Plus3G2;
...
Double_t InverseBetaCS::SigmaTot(Double_t aEnu) {
  ...
  if( aEnu>10 ) {
    ...
    answer = fDifferential->Integral(-1.0, 1.0);
    return answer;
  }
...
void InverseBetaCS::setupTotalCrossSection() {
  ...
  for( int i=1 ; i<=1000 ; i++ ) {
	...
	fDifferential->SetParameter(0,enu);
    fHTotalCrossSection->SetBinContent(i, fDifferential->Integral(-1.0, 1.0) );
...
// Convert neutrino spectrum to visible energy
// NeuSpec and VisSpec must have the same binning and range
int InverseBetaCS::VisIbdSpec(TH1D* NeuSpec, TH1D* VisSpec)
{
  int nbin = NeuSpec->GetNbinsX();
  double eStart = NeuSpec->GetXaxis()->GetXmin();
  double eStop = NeuSpec->GetXaxis()->GetXmax();
  
  // Set up crosssection histogram
  TH1D IbdCs("IbdCs","IbdCs", nbin,eStart,eStop );
  for( int bin = 1; bin <= nbin; bin++ )  {
    double Enu[1], dummy[1];
    Enu[0] = IbdCs.GetBinCenter(bin);
    double cross = gSigmaTotal( Enu, dummy ) * 1e-42; // cm^2
    if( cross<0 ) cross=0; // Why? Negative crosssection?
    IbdCs.SetBinContent( bin, cross );
  }
  
  // Visable spectrum vis IBD process
  TH1D tempIBD( "tempIBD", "tempIBD", nbin,eStart,eStop );
  tempIBD.Multiply( NeuSpec, &IbdCs );

  // Convert to visible energy
  for( int binVis=1; binVis<=nbin; binVis++ )  {
    //  Enu = Eprompt_vis + 0.792 MeV
    double evis = VisSpec->GetBinCenter(binVis);
    double enu = evis+0.792;
    double rate;
    if( enu<eStart || enu>eStop ) {
      rate=0;
    } else {
      int binNu = tempIBD.FindBin( enu );
      rate = tempIBD.GetBinContent( binNu );
    }
    VisSpec->SetBinContent( binVis, rate );
  }
