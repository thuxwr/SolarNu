...
void c10(){
  ...
  TF1 *f3 = new TF1("f3", unique, 0,2.651, 6);
  TF1 *f4 = new TF1("f4", allowed, 0,2.651, 6);  
  f3->SetParameter(0,Z);
  f3->SetParameter(2,A);
  ...
  Double_t binc1[3650];//记录瞬时bincontent
  Double_t binc2[3650];//记录叠加后的bincontent
  //----------初始化-------------
  for(Int_t i=0;i<=3649;i++){
    binc2[i]=0;                                                    
  }
 ...
 //-----------------forbidden-------------------------------      
  ...
  //------读入允许跃迁数据-----------
  Double_t data[2][3];//输入数据，顺序：Br1,logft,Q,Eg,Br2
  ifstream fin("c10all2.dat");  
  for(Int_t m=0;m<=1;m++){
 ...
 //   Double_t Br2=data[m][4];
    ...
    TH1F *h1 = new TH1F("h1","h1",2651,0,2.651);
    h1->Add(f4);
  ...
  TH1F *h2=new TH1F("h2","C10 Beta Sprectrum",2651,0,2.651);
  for(Int_t i=1;i<=2651;i++){
